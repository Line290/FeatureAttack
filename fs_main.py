...
from attack_methods import Attack_FeaScatter, attack_inversion
...
if args.adv_mode.lower() == 'attack_inversion':
    net = basic_net
else:
...
def train_fun(epoch, net):
    ...
    for batch_idx, (inputs, targets) in enumerate(iterator):
        ...
        adv_x, adv_soft_y = attack_inversion(net, inputs, targets, config_feature_scatter)
        net.train()
        # forward
        logits, feat = net(adv_x)

        loss_fs = soft_xent_loss(logits, adv_soft_y)
        optimizer.zero_grad()
        loss = loss_fs.mean()
        loss.backward()
        ...
        if batch_idx % args.log_step == 0:
            net.eval()
            adv_acc = get_acc(logits, targets)

            nat_outputs, _ = net(inputs)
